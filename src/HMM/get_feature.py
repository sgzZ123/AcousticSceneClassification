...
def get_stft_amp(wavdata,fs=44100):                         #extracting speech spectrogram features
	...
	return (np.log10(sub_freq)).astype(np.int32)
...
def get_mel_amp(wavdata,fs=44100):                         #extracting log mel spectrogram features
	...
	melX = np.dot(melW,X)
	return (np.log10(melX)).astype(np.int32)

if __name__ == '__main__':
	wav_file_path = 'data/airport-barcelona-0-0-a.wav'                        #audio file
	...
	mel_amp = mel_amp.astype(np.int32)
	print(mel_amp.shape)
